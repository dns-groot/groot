...
BOOST_AUTO_TEST_CASE(demo_test)
{
    ...
    std::ifstream metadataFile((directory / "net.sy" / "zone_files" / "metadata.json").string());
    json metadata;
    metadataFile >> metadata;

    std::ifstream i((directory / "net.sy" / "jobs.json").string());
    json j;
    i >> j;

    long total_rrs_parsed = driver.SetContext(metadata, (directory / "net.sy" / "zone_files").string());
