...
inline void interpretation::Graph::VertexWriter<NSMap, QueryMap, AnswerMap>::operator()(ostream& out, const Vertex& v) const
{
...
boost::optional< interpretation::Graph::VertexDescriptor> interpretation::Graph::InsertNode(string ns, EC query, VertexDescriptor edge_start, boost::optional<VertexDescriptor> edge_query)
{
	...
	if (it == nameserver_to_vertices_map_.end()) {
		...
		for (auto& n : existingNodes) {
			if (query == (*this)[n].query) {
				...
				if (!b) {
					Logger->critical(fmt::format("interpretation-graph.cpp (InsertNode) - Unable to add edge to a interpretation graph"));
					std::exit(EXIT_FAILURE);
				}
				if (edge_query) {
	...
	if (it != nameserver_to_vertices_map_.end()) {
		...
		Logger->critical(fmt::format("interpretation-graph.cpp (InsertNode) -Unable to insert into a nameserver_to_vertices_map_"));
		std::exit(EXIT_FAILURE);
	}
}
...
vector<tuple<ResourceRecord, vector<ResourceRecord>>>  interpretation::Graph::MatchNsGlueRecords(vector<ResourceRecord> records) const
{
...
void interpretation::Graph::NsSubRoutine(const VertexDescriptor& current_vertex, const string& new_ns, boost::optional<VertexDescriptor> edge_query, const Context& context)
{
	...
	if (node) {
		...
		if (it != context.nameserver_zoneIds_map_.end()) {
			...
			vector<zone::LookUpAnswer> answer;
			answer.push_back(std::make_tuple(ReturnTag::NSNOTFOUND, (*this)[node.get()].query.rrTypes, vector<ResourceRecord> {}));
			(*this)[node.get()].answer = answer;
		}
	}
}

void interpretation::Graph::PrettyPrintLoop(const VertexDescriptor& start, Path p, moodycamel::ConcurrentQueue<json>& json_queue) const
{
	Path loop;
	bool found = false;
	for (auto v : p) {
...
EC interpretation::Graph::ProcessDname(const ResourceRecord& record, const EC query) const
{
	...
	for (auto& l : record.get_name()) {
		if (!(l == query.name[i])) {
			Logger->critical(fmt::format("interpretation-graph.cpp (ProcessDname) - Query name is not a sub-domain of the DNAME record"));
			exit(EXIT_FAILURE);
		}
		i++;
	}
		...
		if (zone_lookup_answers.size() > 0) {
			for (auto& answer : zone_lookup_answers) {
				...
				if (ret == ReturnTag::ANS) {
					...
					vector<tuple<ResourceRecord, vector<ResourceRecord>>> pairs = MatchNsGlueRecords(std::get<2>(answer));
					for (auto& pair : pairs) {
...
interpretation::Graph::VertexDescriptor interpretation::Graph::SideQuery(const EC query, const Context& context)
{
	...
	if (it != nameserver_to_vertices_map_.end()) {
		...
		Logger->critical(fmt::format("interpretation-graph.cpp (SideQuery) - Unable to insert into a nameserver_to_vertices_map_"));
		std::exit(EXIT_FAILURE);
	}
...
void interpretation::Graph::CheckForLoops(VertexDescriptor current_vertex, Path p, moodycamel::ConcurrentQueue<json>& json_queue) const
{
...
void interpretation::Graph::CheckPropertiesOnEC(const vector<interpretation::Graph::PathFunction>& path_functions, const vector<interpretation::Graph::NodeFunction>& end_node_functions, moodycamel::ConcurrentQueue<json>& json_queue) const
{
...
void interpretation::Graph::EnumeratePathsAndReturnEndNodes(VertexDescriptor current_vertex, vector<VertexDescriptor>& end_nodes, Path p, const vector<interpretation::Graph::PathFunction>& path_functions, moodycamel::ConcurrentQueue<json>& json_queue) const
{
	const EC& query = (*this)[current_vertex].query;
	if ((*this)[current_vertex].ns != "") {
		...
		if ((*this)[current_vertex].answer && (*this)[current_vertex].answer.get().size() > 0) {
			if (std::get<0>((*this)[current_vertex].answer.get()[0]) == ReturnTag::REWRITE && std::get<2>((*this)[current_vertex].answer.get()[0])[0].get_type() == RRType::CNAME && query.rrTypes[RRType::CNAME]) {
			...
			Logger->warn(fmt::format("interpretation-graph.cpp (EnumeratePathsAndReturnEndNodes) - Empty Answer found at a node in the interpretation graph for {}", query.ToString()));
		}
	}
	for (auto& v : p) {
		if (v == current_vertex) {
