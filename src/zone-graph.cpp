...
zone::Graph::VertexDescriptor zone::Graph::AddNodes(zone::Graph::VertexDescriptor closest_encloser, const vector<NodeLabel>& labels, const int& index)
{
...
zone::Graph::VertexDescriptor zone::Graph::GetAncestor(zone::Graph::VertexDescriptor closest_encloser, const vector<NodeLabel>& labels, int& index) const {
	vector<VertexDescriptor> vertices_to_create_maps{};
	return GetAncestor(closest_encloser, labels, vertices_to_create_maps, index);
}

zone::Graph::VertexDescriptor zone::Graph::GetAncestor(zone::Graph::VertexDescriptor closest_encloser, const vector<NodeLabel>& labels, vector<VertexDescriptor>& vertices_to_create_maps, int& index) const {
...
zone::Graph::VertexDescriptor zone::Graph::GetClosestEncloser(zone::Graph::VertexDescriptor closest_encloser, const vector<NodeLabel>& labels, int& index) const {
		...
		if (query.excluded) {
			...
			for (VertexDescriptor v : boost::make_iterator_range(adjacent_vertices(closest_encloser, *this))) {
				if ((*this)[v].name.get() == "*") {
					...
					if (query_types_found.count())answers.push_back(std::make_tuple(ReturnTag::ANS, query_types_found, matching_rrs));
					if ((query_types_found ^ query.rrTypes).count()) {
						answers.push_back(std::make_tuple(ReturnTag::ANS, query_types_found ^ query.rrTypes, vector<ResourceRecord> {}));
					}
					return boost::make_optional(answers);
				}
			}
...
bool zone::Graph::RequireGlueRecords(const vector<ResourceRecord>& ns_records) const
{
	for (auto& record : ns_records) {
		vector<NodeLabel> ns_name = LabelUtils::StringToLabels(record.get_rdata());
		if (ns_name.size() < origin_.size()) continue;
		int i = 0;
